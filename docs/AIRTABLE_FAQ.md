...
Your base has **nine tables**. Four of them are the *engine* — the Grant Assistant's skills read from and write to them automatically, every time they run. Five of them are *yours* — built for you to shape around how your organization actually manages its work, and no automation ever touches them.
...
| **Org Profile** | Who your org is — the facts the skills use to judge fit | No structural changes — but you edit its *content* constantly |
| **Search Audit Log** | An automatic record of every scan and scoring session | No — this is the engine's own notebook |
| **Active Grants** | Your team's day-to-day work on grants you're pursuing | **Yes — customize freely** |
| **Portfolio** | Managing grants you've actually won | **Yes — customize freely** |
...
Grant Master, Donor Master, Org Profile, and Search Audit Log are what your five installed skills actually read and write. The skills find data by looking for specific field names and specific option values (like the word "Possible" in a Stage field). If a field gets renamed, deleted, or its dropdown options get changed, the skill that depends on it can quietly stop working — or worse, silently misbehave — without any obvious error message.

That's the *only* reason these four are off-limits for structural changes. It has nothing to do with these tables being more important than the other five — it's that automation is watching them closely, and the other five have no automation watching them at all.

**One important distinction: "don't restructure" is not the same as "don't touch."** You will use the data in these tables all the time — reading Grant Master to see your pipeline, updating Org Profile as your organization grows. What you shouldn't do is add, remove, or rename *fields*, or add/remove *options* inside a dropdown field, in any of these four.

### Grant Master

**What it is:** One row per grant opportunity, covering its entire life — from the moment your prospect scan finds it, through your team pursuing it, to winning or losing it. A single field called **Stage** tracks where it is right now: `Possible`, `Active`, `Awarded`, `Archived`, or `Withdrawn`.
...
| Stage | Possible / Active / Awarded / Archived / Withdrawn — where this grant is in its life right now |
...
**Starting fields:** Owner, Internal Deadline, Application Status (Drafting / Internal Review / Submitted / Awaiting Decision), Work Notes.
...
**Some views worth building right away:**
- In Grant Master: a view showing only Stage = Possible, sorted by Fit Score, so your weekly review only shows what's actually awaiting a decision.
- In Portfolio: a view sorted by Reporting Due, so you never miss a report deadline.
- In Active Grants: a kanban board grouped by Application Status, so your team can see the whole pipeline at a glance.
...
- Recording something new → add a field (only in one of your five customizable tables).
...
The fastest way to trust that you understand this is to actually do it. Both exercises below are on your five customizable tables, so there's no way to cause a problem — the absolute worst case is you delete a view or a field you just made, which takes one click.

### Exercise 1 — Build a view

**Goal:** In Portfolio, build a view that shows only grants with a report due in the next 30 days.

1. Open your base and click into the **Portfolio** table.
...
| Org Profile | Don't add/rename/remove fields | **Edit the content often** — this is how you keep the system current |
...
If you're ever unsure whether a table is safe to reshape, come back to this table — or just remember: **if a skill depends on it, leave the structure alone; if it's one of your five, it's yours.**
